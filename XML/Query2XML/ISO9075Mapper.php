...
class XML_Query2XML_ISO9075Mapper
{
        ...
        */
        for ($i = 0; $i < count($S); $i++) {
            if (self::_unicodeToUtf8($S[$i]) == ':') {
                ...
                * we always do a full escape - therefore we do not have to implement iii)
                */
                
            } elseif ($i < count($S) - 1 && self::_unicodeToUtf8($S[$i]) == '_' && self::_unicodeToUtf8($S[$i+1]) == 'x') {
                /*
            ...
            } elseif (!self::_isValidNameChar($S[$i]) || $i == 0 && !self::_isValidNameStartChar($S[$i])) {            
                /*
        ...
        if (count($S) >=3 && strpos(strtolower(self::_unicodeToUtf8($S[0]) . self::_unicodeToUtf8($S[1]) . self::_unicodeToUtf8($S[2])), 'xml') === 0) {
            /*
    ...
    * @param int $char A unicode character as an integer.
    * @return boolean Wheather $char is a valid NameStartChar.
    */
