<?php
/**This file contains the class Query2XMLTestADOdbException.
*
* LICENSE:
...
// Call Query2XMLTestADOdbException::main() if this source file is executed directly.
if (!defined("PHPUnit2_MAIN_METHOD")) {
    define("PHPUnit2_MAIN_METHOD", "Query2XMLTestADOdbException::main");
}
...
/**Test class for XML_Query2XML that requires ADOdb.
* All tests are run by using exceptions for error handling
...
* @copyright Empowered Media 2006
*/
class Query2XMLTestADOdbException extends PHPUnit2_Framework_TestCase
{
    ...
    */
    public static function main() {
        ...
        $suite  = new PHPUnit2_Framework_TestSuite("Query2XMLTestADOdbException");
    ...
    {        
        ...
        $this->query2xml =& XML_Query2XML::factory($adodb);
    }
    
    public function testFactoryNotConnectedException()
    {
        try {
            require_once('adodb/adodb.inc.php');
            $adodb = ADONewConnection('mysql');
    ...
    public function testFactoryNotConnected2Exception()
    {
        try {
            require_once('adodb/adodb.inc.php');
            $adodb = ADONewConnection('mysql');
            @$adodb->Connect('256.256.256.256', 'bogususer', '', 'bogusdb');
            $query2xml =& XML_Query2XML::factory($adodb);
            self::assertTrue(false);
        } catch (XML_Query2XML_DBException $e) {
    ...
    public function test_prepareAndExecuteExecuteQueryDBException()
    {
        try {
            ...
            self::assertEquals(
                'Could not execute the following SQL query: SELECT * FROM '
                . 'non_existing_table; mysql error: [1146: Table \'query2xml_tests'
                . '.non_existing_table\' doesn\'t exist] in EXECUTE("SELECT * FROM'
                . ' non_existing_table")' . "\n",
                $e->getMessage()
            );
        }
    ...
    public function test_prepareAndExecuteSimpleQueryDBException()
    {
        try {
            ...
            self::assertEquals(
                'Could not run the following SQL query: SELECT * FROM '
                . 'non_existing_table; mysql error: [1146: Table \''
                . 'query2xml_tests.non_existing_table\' doesn\'t exist'
                . '] in EXECUTE("SELECT * FROM non_existing_table")' . "\n",
                $e->getMessage()
            );
        }
    }
    ...
    public function test_mapSQLIdentifierToXMLNameNotCallableException3()
    {
        try {
            ...
            self::assertEquals(
                '[elements][albums]: The method/function "Query2XMLTestADOdbException::noSuchMethod" specified in the configuration option "mapper" is not callable.',
                $e->getMessage()
            );
        }
    }
...
// Call Query2XMLTestADOdbException::main() if this source file is executed directly.
if (PHPUnit2_MAIN_METHOD == "Query2XMLTestADOdbException::main") {
    Query2XMLTestADOdbException::main();
}
?>